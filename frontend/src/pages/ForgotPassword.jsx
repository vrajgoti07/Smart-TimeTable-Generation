...
import { Mail, ArrowRight, KeyRound, CheckCircle, Sparkles, AlertTriangle, ShieldCheck } from 'lucide-react';
...
export default function ForgotPassword() {
    ...
    const features = [
        { icon: ShieldCheck, text: 'Secure Account Recovery' },
        { icon: KeyRound, text: 'Instant Password Reset' },
        { icon: CheckCircle, text: '24/7 Support Access' },
    ];
    ...
    return (
        <div className="min-h-screen w-full flex bg-slate-50 font-sans overflow-hidden relative">
            ...
            <div className="w-full lg:w-1/2 flex flex-col justify-center items-center px-6 sm:px-12 lg:px-24 py-12 relative z-10">
                ...
                <div className="w-full max-w-md bg-white border border-transparent shadow-2xl shadow-slate-200/50 rounded-3xl p-8 sm:p-10 animate-scale-in">
                    ...
                    <div className="flex flex-col items-center mb-10 text-center">
                        ...
                        <div>
                            <span className="text-2xl font-bold text-slate-900 tracking-tight block">Forgot Password?</span>
                            <span className="text-sm text-emerald-600 font-semibold tracking-wide mt-1 block">Account Recovery</span>
                        </div>
                    </div>
                    ...
                    {!isSuccess ? (
                        <>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <button
                                    type="submit"
                                    disabled={isLoading}
                                    className="w-full flex justify-center py-4 px-4 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 border border-transparent rounded-xl shadow-lg shadow-emerald-500/30 text-sm font-bold text-white transition-all duration-300 transform hover:-translate-y-1 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 relative overflow-hidden animate-slide-up stagger-2 disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none"
                                >
                                    {/* Shimmer Overlay */}
                                    {!isLoading && (
                        ...
                        <div className="text-center space-y-6 animate-scale-in">
                            <div className="p-6 bg-emerald-50 text-emerald-700 rounded-2xl border border-emerald-100 shadow-sm">
                                <div className="w-12 h-12 bg-emerald-100 rounded-full flex items-center justify-center mx-auto mb-3">
                                    <CheckCircle className="w-6 h-6 text-emerald-600" />
                                </div>
                                <h3 className="font-bold text-lg mb-1">Check your email</h3>
                                <p className="text-sm">We've sent a password reset link to <span className="font-bold block mt-1">{email}</span></p>
                            </div>
                            <p className="text-sm text-slate-500">
                                Didn't receive the email? <button onClick={() => setIsSuccess(false)} className="text-emerald-600 font-semibold hover:text-emerald-700 hover:underline transition-colors">Click to resend</button>
                            </p>
                        </div>
                    )}

                    <div className="mt-8 text-center animate-slide-up stagger-3">
                        <Link to="/login" className="inline-flex items-center text-sm font-semibold text-slate-500 hover:text-emerald-600 transition-colors group">
            ...
            <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden items-center justify-center p-12">
                ...
                {/* Floating cards decoration - Reused from Login but slightly different arrangement or same for consistency */}
                ...
                <div className="max-w-md relative z-10 text-white">
                    ...
                    <p className="text-lg text-slate-400 mb-10 leading-relaxed animate-slide-up" style={{ animationDelay: '0.2s' }}>
                        We'll help you get back on track. Just enter your email and we'll send you a link to reset your password.
                    </p>
